...
def wait_processing():
    while True:
        time.sleep(0.1)
        content = pyautogui.locateOnScreen(
            'libs/processing_img.png', region=(21, 646, 80, 18), confidence=0.9)
        if content == None:
            break
...
# dengan fitur 3 layer sl
def order_sellSL(kode, price, lot, live=False):
    new_price = price
    for i in range(3):
        pyautogui.click(x=77, y=105)  # click tab automatic order
